...
def process_box_no_anchor(boxes, labels, img_size, class_num):
    '''
    ...
    # [13, 13, 5 + num_class + 1] `5` means coords and labels. `1` means mix up weight. 
    y_true_13 = np.zeros((img_size[1] // 32, img_size[0] // 32, 1, 6 + class_num), np.float32)
    y_true_26 = np.zeros((img_size[1] // 16, img_size[0] // 16, 1, 6 + class_num), np.float32)
    y_true_52 = np.zeros((img_size[1] //  8, img_size[0] //  8, 1, 6 + class_num), np.float32)

    # set all boxes default to 1.
    y_true_13[:, :, 0, 0:4] = 1.
    y_true_26[:, :, 0, 0:4] = 1.
    y_true_52[:, :, 0, 0:4] = 1.
    ...
    # gentrate ture label in 3 layers and use right distance 
    # to allocate the overlap area class label
    for index, down_step in enumerate([32, 16, 8]):
        for numth in range(boxes.shape[0]):
            
            # original area in feature map
            box_down_i = boxes[numth, :-1] / down_step
 
            center_x = ((boxes[numth, 0] + boxes[numth, 2]) / 2) / img_size[0]
            center_y = ((boxes[numth, 1] + boxes[numth, 3]) / 2) / img_size[1]
            width    = (boxes[numth, 2] - boxes[numth, 0]) / img_size[0]
            height   = (boxes[numth, 3] - boxes[numth, 1]) / img_size[1]
            width_exp  = width * math.exp(0 - width)
            height_exp = height * math.exp(0 - height)
            if(width < 0.2):
                width_exp = width
            if(height < 0.2):
                height_exp = height
            x_l_exp = (center_x - width_exp / 2)  * img_size[0]
            x_r_exp = (center_x + width_exp / 2)  * img_size[0]
            y_t_exp = (center_y - height_exp / 2) * img_size[1]
            y_d_exp = (center_y + height_exp / 2) * img_size[1]
            
            # effective area in feature map
            box_down_i_exp = np.array([x_l_exp, y_t_exp, x_r_exp, y_d_exp]) / down_step
            # class label
            c = labels[numth]
            # mix_up weight
            mx_w = boxes[numth, -1]
            generate_feature_label(y_true[index], boxes[numth], box_down_i, box_down_i_exp, c, mx_w, img_size)

    return y_true[0], y_true[1], y_true[2]


def process_box_anchor(boxes, labels, img_size, class_num, anchors):
    '''
    Generate the y_true label, i.e. the ground truth feature_maps in 3 different scales.
    params:
        boxes: [N, 5] shape, float32 dtype. `x_min, y_min, x_max, y_mix, mixup_weight`.
...
def generate_feature_label(y_true_i, box, box_down_i, box_down_i_exp, c, mx_w, img_size):
    x_min = int(box_down_i[0]) if box_down_i[0] < (int(box_down_i[0]) + 0.5) else int(box_down_i[0]) + 1
    x_max = int(box_down_i[2]) if box_down_i[2] > (int(box_down_i[2]) + 0.5) else int(box_down_i[2]) - 1
    y_min = int(box_down_i[1]) if box_down_i[1] < (int(box_down_i[1]) + 0.5) else int(box_down_i[1]) + 1
    y_max = int(box_down_i[3]) if box_down_i[3] > (int(box_down_i[3]) + 0.5) else int(box_down_i[3]) - 1

    x_min_exp = int(box_down_i_exp[0]) if box_down_i_exp[0] < (int(box_down_i_exp[0]) + 0.5) else int(box_down_i_exp[0]) + 1
    x_max_exp = int(box_down_i_exp[2]) if box_down_i_exp[2] > (int(box_down_i_exp[2]) + 0.5) else int(box_down_i_exp[2]) - 1
    y_min_exp = int(box_down_i_exp[1]) if box_down_i_exp[1] < (int(box_down_i_exp[1]) + 0.5) else int(box_down_i_exp[1]) + 1
    y_max_exp = int(box_down_i_exp[3]) if box_down_i_exp[3] > (int(box_down_i_exp[3]) + 0.5) else int(box_down_i_exp[3]) - 1
    
    # get the ground truth bboxes's 4 coordiantes related to the full image size
    center_x = ((box[0] + box[2]) / 2) / img_size[0]
    center_y = ((box[1] + box[3]) / 2) / img_size[1]
    width    = (box[2] - box[0]) / img_size[0]
    height   = (box[3] - box[1]) / img_size[1]
    
    # calaucate the boxes's right edge distance
    r_distance = center_x + width / 2
    for axis_1 in range(x_min, x_max + 1):
        for axis_0 in range(y_min, y_max + 1):
            exist_r_distance = y_true_i[axis_0, axis_1, 0, 2] / 2 + y_true_i[axis_0, axis_1, 0, 0]
                
            # judge the right distance
            if(r_distance < exist_r_distance):
                # judge if this feature map location in effective location
                if (axis_0 >= y_min_exp and axis_0 <= y_max_exp) and \
                   (axis_1 >= x_min_exp and axis_1 <= x_max_exp):
                    # in effective location
                    y_true_i[axis_0, axis_1, 0, 0] = center_x
                    y_true_i[axis_0, axis_1, 0, 1] = center_y
                    y_true_i[axis_0, axis_1, 0, 2] = width
                    y_true_i[axis_0, axis_1, 0, 3] = height
                    y_true_i[axis_0, axis_1, 0, 4] = 2.
                    y_true_i[axis_0, axis_1, 0, 5 + c] = 1.
                    y_true_i[axis_0, axis_1, 0, -1] = mx_w
                else:
                    # not in effective location, just make a tag
                    y_true_i[axis_0, axis_1, 0, 4] = 1.
...
def parse_data(line, class_num, img_size, anchors, mode, letterbox_resize):
    ...
    y_true_13_no_anchor, y_true_26_no_anchor, y_true_52_no_anchor = process_box_no_anchor(boxes, labels, img_size, class_num)
    y_true_13_anchor, y_true_26_anchor, y_true_52_anchor = process_box_anchor(boxes, labels, img_size, class_num, anchors)
    y_true_13 = np.concatenate((y_true_13_anchor, y_true_13_no_anchor), axis=-2)
    y_true_26 = np.concatenate((y_true_26_anchor, y_true_26_no_anchor), axis=-2)
    y_true_52 = np.concatenate((y_true_52_anchor, y_true_52_no_anchor), axis=-2)
